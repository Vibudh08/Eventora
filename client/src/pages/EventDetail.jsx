  ...
  useEffect(() => {
    const fetchEvent = async () => {
      try {
        ...
        if (user) {
          const bookingsRes = await api.get("/booking/me");
          const existingBooking = (bookingsRes.data.bookings || []).find(
            (booking) => booking.eventId?._id === data._id,
          );
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      if (booking.amount === 0) {
        ...
        try {
          await api.put(`/booking/${booking._id}/confirm`, {});
          setSuccessMsg("Booking confirmed successfully");
       ...
       handler: async function (response) {
         ...
         const { data: verifyData } = await api.post(
          ...
          try {
            ...
            setError(
              confirmError.response?.data?.message ||
                "Payment succeeded, but booking could not be confirmed. Please contact support.",
            );
          } finally {
            setConfirmingBooking(false);
          }
        ...
        <div className="flex flex-col md:flex-row justify-between items-start mb-8 gap-6">
          ...
          <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 min-w-[300px] w-full md:w-auto shrink-0 shadow-sm">
            ...
            <button
              ...
              disabled={
                isSoldOut || bookingLoading || confirmingBooking || successMsg
              }
