...
const AdminDashboard = () => {
    ...
    const fetchData = async () => {
        try {
            const { data } = await api.get('/booking/admin/events-summary?type=all');
            const userBookings = (data.users || []).flatMap((bookingUser) =>
                (bookingUser.bookings || []).map((booking) => ({
                    ...booking,
                    userId: {
                        _id: bookingUser._id,
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
                <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 flex items-center justify-between">
                    <div>
                        ...
                        <h3 className="text-3xl font-black text-green-600">₹{bookings.reduce((sum, b) => b.paymentStatus === 'paid' && b.status === 'booked' ? sum + b.amount : sum, 0)}</h3>
                    </div>
                ...
                <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 mb-8 animation-slideDown">
                    ...
                    <form onSubmit={handleCreateEvent} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div className="md:col-span-2">
                            <input required type="text" placeholder="Image URL (Provide any direct link to an image)" className="w-full border px-4 py-3 rounded-lg focus:ring-2 focus:ring-gray-700 outline-none transition" value={formData.image} onChange={e => setFormData({ ...formData, image: e.target.value })} />
                        </div>
